...
const Sidebar = () => {
  ...
  return noSidebar.includes(location.pathname) ? (
    ...
    <div style={{ zIndex: 999 }} className="fixed left-0 top-0 ">
      ...
      >
        <div
          className="absolute top-1/2 left-[29px] -translate-y-1/2 h-[450px] w-[55px] rounded-3xl"
          style={{
            background:
        ...
        >
          ...
          <ul className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2">
            {Menus.map((Menu, index) => (
              <button
                onClick={() => navigate(Menu.onClick)}
                key={index}
                className={`w-[55px] flex group ${location.pathname == Menu.onClick ? "bg-bgl2 w-[40px]" : ""} relative rounded-md p-2 cursor-pointer ${
                  !open ? "mx-auto" : ""
